...
variable "environments" {
  description = "Github Environments configuration for the repository. In deployment_branch_policy, both protected_branches and custom_branch_policies cannot be set to true at the same time."
  type = map(object({
    ...
    env_secrets                = optional(map(any), {})
    branch_deployment_policies = optional(list(string), [])
    tag_deployment_policies    = optional(list(string), [])
    })
  )
  default = {}
  validation {
    condition = alltrue([
      ...
      !(cfg.deployment_branch_policy.protected_branches && cfg.deployment_branch_policy.custom_branch_policies)
    ])
    error_message = "Cannot set both protected_branches and custom_branch_policies to true in any environment."
  }
}